...
class WrapParameters(Parameters):
    ...
    downsample=False
    # Cross section model
    #: *(int) For the built-in ``pytb.input.He_TB`` Helium model, include CCC excitation
    #: processes from the ground state to (up to and including) states with
    #: principle quantum number ``n``.
    n=3
    #: *(str or bool) For the built-in ``pytb.input.He_TB`` Helium model, use
    #: either tabulated data, analytic fits, or a mix of both,
    #: options are ``False`` | ``True`` | ``"mixed"``.
    analytic_cs="mixed"
    #: *(int) For the built-in ``pytb.input.He_TB``, this specifies
...
class OutputParameters(Parameters):
    ...
    returned by :meth:`~.pytb.ThunderBoltz.get_timeseries` and
    :meth:`~.pytb.ThunderBoltz.get_ss_params` respectively. These data tables also
    include the :class:`ParticleParameters` of the species at index :math:`0`.
    The steady state parameters returned by :meth:`~.pytb.ThunderBoltz.get_ss_params`
...
class ParticleParameters(Parameters):
    ...
    :meth:`~.pytb.ThunderBoltz.get_particle_tables`, which returns a list of data tables
